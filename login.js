...
const file = multer.diskStorage({
    destination: (req, file, calb) => {
        calb(null, "src/assets/upload")
    },
...
login.route('/homelogin')
    .get(async function (req, res) {
        try {
            const query = await SequelizePool.query("select * from blogs order by blogs desc", {
                type: QueryTypes.SELECT
            })
...
login.route('/bloglogin')
    .get(async function (req, res) {
        try {
            const query = await SequelizePool.query("select blogs.id,projectname,datestarterror,dateenderror,durasi,tech,tech1,description,image, users.name from blogs left join users on blogs.author = users.id order by blogs desc ", {
                type: QueryTypes.SELECT
            })
            const dataBlog = query.map(res => ({
                ...res,
    ...
    .post(upload.single('image'), async function (req, res) {
        try {
            ...
            const query = await SequelizePool.query(`insert into blogs (projectname,datestarterror,dateenderror,durasi,tech,tech1,description,image,author) values ('${projectName}','${dateStartError}','${dateEndError}','${durasi}','{${tech}}','{${tech1}}','${description}','${image}','${author}')`)
...
login.route('/editDetailLogin/:id')
    ...
    .post(upload.single('imageEdit'), async function (req, res) {
        try {
            const {
                projectName,
                startDate,
